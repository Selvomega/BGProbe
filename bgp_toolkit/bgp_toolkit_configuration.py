...
@dataclass
class BGPToolkitConfiguration:
    """
    You can use this class to configure the BGP toolkit.
    """
...
def parse_bgp_config_from_yaml(file_path: str) -> BGPToolkitConfiguration:
    """
    Parse a YAML file and return a BGPToolkitConfiguration instance.
    
    Args:
        file_path: Path to the YAML configuration file
        
    Returns:
        BGPToolkitConfiguration instance populated from the YAML file
    """
    with open(file_path, 'r') as file:
        config_data = yaml.safe_load(file)
    
    return BGPToolkitConfiguration(
        asn=config_data['asn'],
