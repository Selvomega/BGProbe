...
def main(test_id: int, repeat_num: int, test_name: str = None):
    """
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--number", "-n",
        type=int, 
        help=f"Testcase id between 0-{len(repeated_testcase_suite)-1}",
        choices=range(0, len(repeated_testcase_suite))
    )
    parser.add_argument(
        "--repeat", "-r",
        type=non_negative_int, 
        help=f"Number of times to repeat the execution.",
    )
    parser.add_argument(
        "--test_name",
        type=str,
    ...
    main(test_id=args.number, repeat_num=args.repeat, test_name=args.test_name)
